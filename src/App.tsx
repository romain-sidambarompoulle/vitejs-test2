...
const ODIA_FIELDS = [
  ...
  { 
    key: "revenu_apres_impot_a_investir_SELARL", 
    label: "Revenu après impôt à investir (SELARL)" 
  },
  { key: "total_revenu", label: "Épargne disponible SELARL" },
  { key: "performance_montage", label: "Performance du montage" },
  { 
    key: "capital_revenu_place_15ans", 
    label: "Capital investi après 15 ans" 
  },
  { 
    key: "interet_mensuel_capital_15ans", 
    label: "Intérêts mensuels après 15 ans" 
  },
];
                  ...
                  <List>
                    {BNC_FIELDS.map((field) => (
                      <RegimeItem
                        key={field.key}
                        label={field.label}
                        value={data.simulation_bnc[field.key]}
                      />
                    ))}
                  </List>
                </Paper>
                  ...
                  <List>
                    {ODIA_FIELDS.map((field) => (
                      <RegimeItem
                        key={field.key}
                        label={field.label}
                        value={data.simulation_odia[field.key]}
                      />
                    ))}
                  </List>
                </Paper>
